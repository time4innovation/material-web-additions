...
// @ts-ignore
import { LinearProgress } from 'material/progress/progress.js';
...
export class DataTable extends LitElement {
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <slot
                                    name="header-cell"
                                    @slotchange=${this.onHeaderCellSlotChange}
                                ></slot>
                            </div>
                        </div>
                        <div
    ...
    protected onHeaderCellSlotChange() {
        ...
        if (sortColumn) {
            // @ts-ignore
            sortColumn.onSortButtonClicked(
                new CustomEvent<{
                    selected: boolean;
    ...
    protected filterColumnCallback = (e: Event) => {
        ...
        this.dispatchEvent(
            new CustomEvent<FilteredDetail>('filtered', {
                detail: {
    ...
    protected sortColumnCallback = (e: Event) => {
        ...
        this.dispatchEvent(
            new CustomEvent<SortedDetail>('sorted', {
                detail: {
                    column: event.detail.column,
                    isDescending,
                },
          ...
          <md-data-table-footer>
              <div class="mdc-data-table__pagination-trailing">
                  <div class="mdc-data-table__pagination-rows-per-page">
                      ...
                      <md-select color="outlined" type="number" class="mdc-data-table__pagination-rows-per-page-select" @input=${this.onPageSizeSelected}>
                          ${this.pageSizesArray.map(
                              (size) => html`
                                  <md-select-option
                                      value="${size}"
    ...
    protected onPaginationButtonClicked(event: Event) {
        ...
        this.dispatchEvent(
            new CustomEvent<PageChangeDetail>('page-changed', {
                detail: {
                    action,
                },
            }),
    ...
    paginate(action: PaginationAction = 'current') {
        ...
        this.dispatchEvent(
            new CustomEvent<PaginateDetail>('paginate', {
                detail: {
                    pageSize: this.currentPageSize,
    ...
    protected createAdapter(): MDCDataTableAdapter {
        ...
        return {
            ...
            isRowsSelectable: () =>
                this.headerCheckbox !== undefined || this.rows.filter((row) => row.checkboxCell !== undefined).length > 0,
            notifyRowSelectionChanged: (data: MDCDataTableRowSelectionChangedEventDetail) => {
                this.dispatchEvent(
                    new CustomEvent<RowSelectionChangedDetail>('row-selection-changed', {
                        detail: {
